...
def test_sequence(game):
    ...
    while game.res == ResDom.PLAYING:
        if game.status == Status.TURN_USER:
            # User makes a move
            game.uSelRow = 0
            # Select a random column game.uSelCol which is not full (i.e. has at least an EMPTY cell in row 0)
            game.uSelCol = random.choice([c for c in range(3) if game.board[0][c] == Sign.EMPTY])
        ...
        if game.action == ActionDomain.U_MOVE:
            ...
            try:
                ...
                # assert that in the highest non-empty row of the selected column, the cell is filled with CROSS
                ...
                # assert that all empty_cells from before are still empty except the highest non-empty cell in the selected column
        ...
        try:
            ...
            # assert that there are no EMPTY cells below any CROSS or NOUGHT cells
